...
export default function Game() {
    ...
    const [keyCollected, setKeyCollected] = useState(null);
    ...
    useEffect(() => {
        setKeyPosition(Math.floor(Math.random() * 3) + 12);
    }, []);
    ...
    useEffect(() => {
        ...
        if(selectedPlatform === 'start' && opponentPlatform !== 'opponentStart') {
            addScore = setTimeout(() => {
    ...
    useEffect(() => {
        ...
        if (playerMoved & selectedPlatform !== 'endzone' & selectedPlatform !== 'start') {
            ...
            if (parseInt(selectedPlatform.replace('p', '')) === keyPosition) {
                setKeyPosition(null);
    ...
    const getGem = () => {
        if (['p1','p2','p3','p4'].includes(selectedPlatform) && keyCollected) {
                        ...
                        return (data.platforms && row !== 'start' && row!== 'finish') && <View key={row} style={{...styles.platformRow, ...styles[rowSize]}}>
                            {
                                data.platforms.map(pNum => {
                                    const pNumber = parseInt(pNum.replace('p', ''));
                                    const isDisabled = !PlatformData[currentPos].validMoves.includes(pNumber) || platformsDisabled;
                                    return <TouchableOpacity key={pNum} disabled={isDisabled} style={styles.platformWrapper} onPress={() => movePlayer(row, pNum)}>
                                        <ImageBackground source={require('../assets/space-platform.png')} style={styles.platform}>
                                            {pNumber === keyPosition && <Image source={require('../assets/key.png')} style={styles.key}></Image>}
                                        </ImageBackground>
                                    </TouchableOpacity>;
                                })
                            }
                ...
                <ImageBackground source={require('../assets/space-zone.png')} resizeMode="stretch" style={{...styles.zoneBG, top: 0}}>
                    <TouchableWithoutFeedback onPress={getGem}>
                        <View style={{...styles.zone, ...styles.endZone}}>
                            <Image  source={require('../assets/gem-box.png')} style={styles.gemBox} />
